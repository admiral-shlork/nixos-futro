...
```bash
git clone git@github.com:admiral-shlork/nixos-futro.git
```
```bash
git clone https://github.com/admiral-shlork/nixos-futro.git
```

5. To install the OS run the following command from the root folder of the cloned repository:

```bash
sudo nixos-install --root /mnt  --flake .#futro
```
...
```
```bash
sudo nixos-rebuild boot --flake .#futro
```
```bash
nh clean all --keep 2
```
